...
**Thesis:** Parse the V4A envelope once; transform each file as pure text with unique exact matching (`locate_chunks` → `emit_chunks`); commit all-or-nothing. Never apply via `diffy`/`flickzeug`; never silent fuzzy fallback.
